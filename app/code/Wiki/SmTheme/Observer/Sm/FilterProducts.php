...
namespace Wiki\SmTheme\Observer\Sm;
...
{    
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (!$this->moduleManager->isEnabled('Wiki_Quotation')) return;
    	...
    	if($block instanceof \Sm\FilterProducts\Block\FilterProducts){
    	    ...
    	    $html .= $this->getQuoteProductListHtml(
    	    	...
    	    	'Wiki_Quotation/js/catalog/product-list-featured'
			);
    		...
    		if($productCollection){
    		    $html .= $this->getQuoteProductListHtml(
	               ...
	               'Wiki_Quotation/js/catalog/product-list-tabs'
               	);
    		}
    		$block->setIsAddedQuoteCode(true);
    	}
    ...
    public function getQuoteProductListHtml(
    		$productCollection,
    		$params = [],
    		$component = 'Wiki_Quotation/js/catalog/product-list'
	) {
        $productData = [];
