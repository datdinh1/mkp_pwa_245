 ...
 * Vendor repository.
 */
class DashboardRepository implements \Wiki\VendorsApi\Api\DashboardRepositoryInterface
{
    /**
    ...
    protected function getCreditAmount($customerId){
        $creditAccount = ObjectManager::getInstance()->create('Wiki\Credit\Model\Credit');
    ...
    {
        $orderResource = ObjectManager::getInstance()->get('Wiki\VendorsSales\Model\ResourceModel\Order');
    ...
    /**
     * @param array $data
     * @return \Wiki\VendorsApi\Api\Data\Dashboard\CreditChartInterface[]
     */
    protected function prepareCreditChart($data){
        $om = ObjectManager::getInstance();
        $result = [];
        foreach($data as $creditData){
            $creditChart = $om->create('Wiki\VendorsApi\Api\Data\Dashboard\CreditChartInterface');
    ...
    /**
     * @param array $data
     * @return \Wiki\VendorsApi\Api\Data\Dashboard\OrderChartInterface[]
     */
    protected function prepareOrderChart($data){
        $om = ObjectManager::getInstance();
        $result = [];
        foreach($data as $creditData){
            $orderChart = $om->create('Wiki\VendorsApi\Api\Data\Dashboard\OrderChartInterface');
    ...
    /**
     * {@inheritDoc}
     * @see \Wiki\VendorsApi\Api\VendorRepositoryInterface::getDashboardInfo()
     */
    public function getDashboardInfo($customerId, $period = '7d'){
        ...
        $dashboardData = $om->get('Wiki\VendorsApi\Api\Data\Dashboard\DashboardInterface');
        ...
        $graph = $om->create('Wiki\VendorsDashboard\Model\Graph');
