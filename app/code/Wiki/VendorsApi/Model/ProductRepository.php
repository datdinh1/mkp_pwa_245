 ...
 * Vendor repository.
 */
class ProductRepository implements \Wiki\VendorsApi\Api\ProductRepositoryInterface
{
    /**
     * @var Product[]
     */
    ...
    */
    ...
    /**
     * @var \Wiki\VendorsApi\Api\Data\Catalog\ProductSearchResultsInterfaceFactory
     */
     ...
     * @param \Wiki\VendorsProduct\Helper\Data $vendorProductHelper
     * @param \Wiki\VendorsApi\Api\Data\Catalog\ProductSearchResultsInterfaceFactory $searchResultsFactory
    ...
    public function __construct(
        ...
        \Wiki\VendorsApi\Api\Data\Catalog\ProductSearchResultsInterfaceFactory $searchResultsFactory,
     ...
     * @return mixed|\Wiki\VendorsApi\Api\Data\Catalog\ProductSearchResultsInterface
    ...
    public function submit(
        ...
        try {
            if($collection->count()){
                $vendorProductHelper =  $this->objectManager->get('Wiki\VendorsProduct\Helper\Data');
                foreach ($collection->getItems() as $product) {
                    $product->setApproval(\Wiki\VendorsProduct\Model\Source\Approval::STATUS_PENDING)
                        ->getResource()
        ...
        if ($this->vendorProductHelper->isUpdateProductsApproval()) {
            if (!in_array($existProduct->getApproval(), [Approval::STATUS_PENDING, Approval::STATUS_NOT_SUBMITED, Approval::STATUS_UNAPPROVED])) {
                if (sizeof($changedData)) {
                    /*Save changed data*/
                    $update = $om->create('Wiki\VendorsProduct\Model\Product\Update');
                    ...
                    if ($collection->count()) {
                        ...
                        $update->setData([
                            ...
                            'status' => \Wiki\VendorsProduct\Model\Product\Update::STATUS_PENDING
                        ])->save();
                    }
