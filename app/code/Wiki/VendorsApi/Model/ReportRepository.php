 ...
 * Vendor repository.
 */
class ReportRepository implements \Wiki\VendorsApi\Api\ReportRepositoryInterface
{
    /**
     * @var ApiHelper
     */
    ...
    /**
     * @see \Wiki\VendorsApi\Api\ProductRepositoryInterface::getBestSelling()
     */
    public function getBestSelling($customerId, $limit = 5){
        ...
        foreach($collection as $item){
            $bestsellingItem = $om->create('Wiki\VendorsApi\Api\Data\Report\BestsellingInterface');
        ...
        $searchResult = $om->create('Wiki\VendorsApi\Api\Data\Report\BestsellingSearchResultInterface');
    ...
    public function getBestsellingCollection($vendorId, $limit = 5){
        $om = ObjectManager::getInstance();
        /** @\Wiki\VendorsSales\Model\ResourceModel\Order\Collection */
    ...
    /**
     * @see \Wiki\VendorsApi\Api\ReportRepositoryInterface::getMostViewed()
     */
    public function getMostViewed($customerId, $limit = 5){
        ...
        $items = [];
        foreach($collection as $item){
            $bestsellingItem = $om->create('Wiki\VendorsApi\Api\Data\Report\MostViewedInterface');
            $bestsellingItem->setId($item->getId());
            $bestsellingItem->setName($item->getName());
        ...
        $searchResult = $om->create('Wiki\VendorsApi\Api\Data\Report\MostViewedSearchResultInterface');
        $searchResult->setItems($items);
        $searchResult->setTotalCount(sizeof($items));
