...
class Action implements ObserverInterface
{
    ...
    /**
     * @var \Wiki\Core\Model\ResourceModel\Key\Collection
     */
        ...
        if ($action->getRequest()->isGet()) {
            ...
            try {
                if (
                    ...
                    /* Check all installed Wiki extension are activated or not*/
                    if (!is_array($this->_session->getData('Wiki_check_license_data'))) {
                        $errors = $this->checkLicense();
                        $this->_session->setData('Wiki_check_license_data', $errors);
                    } else {
                        $errors = $this->_session->getData('Wiki_check_license_data');
                    }
                    ...
                    /*Show error message on all Wiki page and dashboard*/
                    if (
                        (
                            !$this->_session->getData('Wiki_core_notify') ||
                        ...
                        if ($errorSize == 1) {
                            $this->messageManager->addError(__(
                                ...
                                '<a href="'.$action->getUrl('Wiki/licenses').'">'.__('Manage Licenses').'</a>'
                            ));
                        } elseif ($errorSize > 1) {
                            $this->messageManager->addError(__(
                    ...
                    if (
                        ...
                        $action->getResponse()->setRedirect($action->getUrl('Wiki/licenses'));
                        $this->_actionFlag->set('', \Magento\Framework\App\ActionInterface::FLAG_NO_DISPATCH, true);
                        $this->_session->setData('Wiki_core_notify', true);
                    }
                }
            } catch (\Exception $e) {
    ...
    public function checkLicense()
    {
        ...
        foreach ($modules as $module => $isEnabled) {
            if (strpos($module, 'Wiki_') === false || !$isEnabled) {
                continue;
            }

            /*Check if the Wiki module is activated*/
    ...
    public function getExtensionsList()
    {
        if (!$this->_extensionsList) {
            $om = \Magento\Framework\App\ObjectManager::getInstance();
            $license = $om->create('Wiki\Core\Model\Key');
     ...
     * 
     * @return \Wiki\Core\Model\ResourceModel\Key\Collection
     */
    public function getLicenseCollection()
    {
        if (!$this->_licenseCollection) {
            $om = \Magento\Framework\App\ObjectManager::getInstance();
            $licenseCollection = $om->create('Wiki\Core\Model\Key')->getCollection();
            /* Update all licenses*/
    ...
    {
        return $this->_cacheManager->load('Wiki_license_update');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'Wiki_license_update');
