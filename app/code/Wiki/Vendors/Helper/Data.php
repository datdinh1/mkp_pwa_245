...
class Data extends AbstractHelper
{
    ...
    /**
     * @var \Wiki\Vendors\App\Area\FrontNameResolver
     */
    ...
    public function getNotActiveVendorIds(){
        ...
        $collection = $om->create('Wiki\Vendors\Model\ResourceModel\Vendor\Collection');
        $collection->addAttributeToFilter('status',['neq' => \Wiki\Vendors\Model\Vendor::STATUS_APPROVED]);
    ...
    public function isUsedCustomVendorUrl(){
        return $this->scopeConfig->getValue(\Wiki\Vendors\App\Area\FrontNameResolver::XML_PATH_USE_CUSTOM_VENDOR_URL);
    }

    public function fixPath($vendorId){
        $vendor = $this->_vendorsModel->loadByIdentifier($vendorId);
        $connection = $this->_resource->getConnection();
        $tableName = $this->_resource->getTableName('ves_vendor_config');
        $query = "SELECT * FROM " . $tableName . " WHERE vendor_id = '" . $vendor->getEntityId() . "'";
        $result = $connection->fetchAll($query);
        foreach ($result as $item) {
            if ($item['path'] == 'page/general/description') {
                
                $sql            = "Update " . $tableName . " Set path = 'general/store_information/detail_store' where vendor_id = '" . $vendor->getEntityId() . "'";
