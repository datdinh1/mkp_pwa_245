...
class Fieldset extends \Magento\Framework\View\Element\Template
{
    ...
    /**
     * Get fieldset object
     * @return \Wiki\Vendors\Model\Vendor\Fieldset
     */
        ...
        if (!$this->_fields) {
            ...
            foreach ($this->getFieldset()->getAttributes() as $attribute) {
                ...
                if(isset($renderers[$attributeCode])){
                    ...
                    $field = $this->getLayout()->createBlock('Wiki\Vendors\Block\Account\Create\Fieldset\VendorId')
                        ->setVendorAttribute($attribute)
                        ->setTemplate('account/create/fieldset/vendor_id.phtml');
                }elseif ($attributeCode == 'region') {
                    $field = $this->getLayout()->createBlock('Wiki\Vendors\Block\Account\Create\Fieldset\Region')
                        ->setVendorAttribute($attribute)
                        ->setTemplate('account/create/fieldset/region.phtml');
                } elseif ($attributeCode == 'region_id') {
                    continue;
                } else {
                    $field = $this->getLayout()->createBlock('Wiki\Vendors\Block\Account\Create\Fieldset\Field')
                        ->setVendorAttribute($attribute);
                }
    ...
    public function getFieldRenderers(){
        ...
        $this->_eventManager->dispatch('Wiki_vendors_register_field_renders',['transport' => $transport]);
