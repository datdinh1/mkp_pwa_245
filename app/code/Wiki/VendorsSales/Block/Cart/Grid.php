...
namespace Wiki\VendorsSales\Block\Cart;
...
class Grid extends \Magento\Checkout\Block\Cart\Grid
{
 /**
    ...
    public function isSliptCart(){
        ...
        $helperFactory = $objectManager->create('\Wiki\VendorsSales\Helper\Data');
    ...
    public function getVendorName($vendorId){
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $modelFactory = $objectManager->create('\Wiki\Vendors\Model\Vendor')->load($vendorId);
    ...
    public function getUrlVendorPage($vendorId){
        ...
        if(!$module->isEnabled("Wiki_VendorsPage")) return false;
        $modelFactory = $objectManager->create('\Wiki\Vendors\Model\Vendor')->load($vendorId);
        if(!$vendorId || !$modelFactory->getId()) return false;
        $url = $objectManager->create('\Wiki\VendorsPage\Helper\Data')->getUrl($modelFactory);
        return $url;
    }
}