...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function __construct(
        ...
        \MGS\Brand\Helper\Data $helperData,
        $connectionName = null
    )
    {
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
		...
		if($object->getProductIds()){
            ...
            if ($productData) {
                $newIds = $this->_helperData->getCategoryIdByProducts($productData);
                $oldIds = $this->lookupCategoryIds($object->getId());
                $this->updateMactch($object, $newIds, $oldIds, 'mgs_brand_category', 'category_id');
            }
		}
    ...
    public function lookupCategoryIds($brandId)
    {
        return $this->lookupIds(
            ['column' => 'brand_id', 'value' => (int)$brandId],
            'mgs_brand_category',
            'category_id'
        );
    }

    protected function lookupIds($data, $tableName, $field)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
    ...
    protected function updateMactch($object, $newIds, $oldIds, $tableName, $field) 
    {
        $table = $this->getTable($tableName);
        $insert = array_diff($newIds, $oldIds);
        $delete = array_diff($oldIds, $newIds);
        if ($delete) {
            $where = ['brand_id = ?' => (int)$object->getId(), 'category_id IN (?)' => $delete];
            $this->getConnection()->delete($table, $where);
        }
        if ($insert) {
            $data = [];
            foreach ($insert as $id) {
                $data[] = ['brand_id' => (int)$object->getId(), 'category_id' => (int)$id];
            }
            $this->getConnection()->insertMultiple($table, $data);
        }
    }
